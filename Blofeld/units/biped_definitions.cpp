...
namespace blofeld
{
	...
	TAG_BLOCK(biped_wall_proximity_block, k_maximum_biped_proximity_feelers)
	{
		...
		FIELD( _field_long_enum, "composition mode#if multiple markers share the same name, this specifies how to compose their search values", &biped_wall_proximity_composition_mode ),
	...
	TAG_STRUCT(biped_group)
	{
		...
		FIELD( _field_long_flags, "flags", &biped_definition_flags ),
	...
	TAG_STRUCT(biped_grab_biped_data_struct)
	{
		...
		FIELD( _field_char_enum, "throw biped control mode", &grab_biped_throw_control_modes ),
