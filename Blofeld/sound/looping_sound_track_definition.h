#pragma once

namespace blofeld
{

	extern const char* looping_sound_fade_modes_strings[];
	extern s_string_list_definition looping_sound_fade_modes;
	extern const char* looping_sound_layers_strings[];
	extern s_string_list_definition looping_sound_layers;
	extern const char* looping_sound_track_flags_strings[];
	extern s_string_list_definition looping_sound_track_flags;
	extern const char* looping_sound_track_layer_flags_strings[];
	extern s_string_list_definition looping_sound_track_layer_flags;


	extern s_tag_struct looping_sound_track_block_block_struct;
	extern s_tag_block_definition looping_sound_track_block_block;


} // namespace blofeld

