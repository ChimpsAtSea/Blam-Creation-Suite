...
namespace blofeld
{
	...
	TAG_GROUP(pca_animation, PCA_ANIMATION_TAG)
	{
		...
		FIELD( _field_long_flags, "pcaFlags*", &pca_animation_tag_flags ),
		...
		FIELD( _field_block, "frame data*", &PCAImportedFrameDatablock_block ),
		FIELD( _field_block, "mesh data*", &PCAImportedMeshDataBlock_block ),
	...
	TAG_BLOCK(PCAImportedMeshDataBlock, (8*1024-1))
	{
		...
		FIELD( _field_block, "animations*", &PCAImportedAnimationDataBlock_block ),
		FIELD( _field_explanation, "Vertices" ),
		FIELD( _field_block, "raw blendshape verts*", &raw_blendshape_block_block ),
		FIELD( _field_terminator )
	};
