...
namespace blofeld
{
	...
	TAG_ENUM(water_density_type_enum, 7)
	{
		OPTION("default"),
		OPTION("super_floater"),
		OPTION("floater"),
		OPTION("neutral"),
		OPTION("sinker"),
		OPTION("super_sinker"),
	...
	TAG_ENUM(global_object_change_color_enum, 5)
	{
		OPTION("none"),
		OPTION("primary"),
		OPTION("secondary"),
	...
	TAG_ENUM(object_function_interpolation_mode_enum, 4)
	{
		OPTION("constant velocity"),
		OPTION("linear acceleration"),
		OPTION("damped spring"),
		OPTION("fractional#covers a fixed fraction of the distance to the target on each update"),
	};

	TAG_ENUM(object_function_flags, 6)
	{
		OPTION("invert#result of function is one minus actual result"),
		OPTION("mapping does not controls active#the curve mapping can make the function active/inactive"),
		OPTION("always active#function does not deactivate when at or below lower bound"),
		OPTION("random time offset#function offsets periodic function input by random value between 0 and 1"),
		OPTION("always exports value#when this function is deactivated, it still exports its value"),
		OPTION("turn off with uses magnitude#the function will be turned off if the value of the turns_off_with function is 0"),
	};

	TAG_ENUM(object_abandonment_flags, 2)
	{
		OPTION("Enable In Multiplayer"),
		OPTION("Enable In Single Player"),
	};

	TAG_ENUM(object_attachment_flags, 2)
	{
		OPTION("force always on#when this flag is clear we only create the attachment when the function object function is active or set to empty string"),
		OPTION("effect size scale from object scale"),
	};

	TAG_ENUM(global_rgb_interpolation_flags, 2)
	{
		OPTION("blend in hsv#blends colors in hsv rather than rgb space"),
		OPTION("...more colors#blends colors through more hues (goes the long way around the color wheel)"),
	};

	TAG_ENUM(multiplayer_object_flags, 7)
	{
		OPTION("only visible in editor"),
		OPTION("phased physics in forge"),
		OPTION("valid initial player spawn"),
		OPTION("fixed boundary orientation"),
		OPTION("candy monitor should ignore"),
		OPTION("inherit owning team color"),
		OPTION("boundary volume doesn\'t kill immediately"),
	};

	TAG_ENUM(multiplayer_object_spawn_timer_types, 2)
	{
		OPTION("starts on death"),
		OPTION("starts on disturbance"),
	};

	TAG_ENUM(teleporter_passability_flags, 5)
	{
		OPTION("disallow players"),
		OPTION("allow light land vehicles"),
		OPTION("allow heavy land vehicles"),
		OPTION("allow flying vehicles"),
		OPTION("allow projectiles"),
	};

	TAG_ENUM(ai_size_enum, 7)
	{
		OPTION("default"),
		OPTION("tiny"),
		OPTION("small"),
	...
	TAG_ENUM(ai_properties_flags, 8)
	{
		OPTION("detroyable cover"),
		OPTION("pathfinding ignore when dead"),
		OPTION("dynamic cover"),
		OPTION("non flight-blocking"),
		OPTION("dynamic cover from centre"),
		OPTION("has corner markers!"),
		OPTION("idle when flying"),
		OPTION("consider for interaction"),
	};

	TAG_ENUM(lightmap_shadow_mode_enum, 4)
	{
		OPTION("default"),
		OPTION("never"),
		OPTION("always"),
	...
	TAG_ENUM(sweetener_size_enum, 4)
	{
		OPTION("default"),
		OPTION("small"),
		OPTION("medium"),
	...
	TAG_ENUM(NavMeshCuttingOverrideEnum, 3)
	{
		OPTION("Default"),
		OPTION("Cut"),
		OPTION("NotCut"),
	...
	TAG_ENUM(BooleanOverrideEnum, 3)
	{
		OPTION("Default"),
		OPTION("Yes"),
		OPTION("No"),
	...
	TAG_ENUM(object_definition_flags, 31)
	{
		OPTION("does not cast shadow"),
		OPTION("children do not cast shadow"),
		OPTION("first class child"),
		OPTION("object samples lightprobes only"),
		OPTION("object uses only own static lightmap"),
		OPTION("search cardinal direction lightmaps on failure"),
		OPTION("preserves initial damage owner"),
		OPTION("not a pathfinding obstacle"),
		OPTION("extension of parent#object uses parent\'s markers"),
		OPTION("does not cause collision damage"),
		OPTION("early mover"),
		OPTION("early mover localized physics"),
		OPTION("object scales attachments"),
		OPTION("inherits player\'s appearance"),
		OPTION("non physical in map editor"),
		OPTION("object is always on the ceiling"),
		OPTION("sample enviroment lighting only ignore object lighting"),
		OPTION("effects created by this object do not spawn objects in multiplayer"),
		OPTION("does not collide with camera#force camera not to collide with object.  By default small sweetener objects do not collide"),
		OPTION("force collide with camera#force the camera to collide with this object,  By default small sweetener objects do not collide"),
		OPTION("damage not blocked by obstructions#AOE damage being applied to this object does not test for obstrutions."),
		OPTION("does not damage breakable surfaces"),
		OPTION("early mover localize projectiles"),
		OPTION("requires shadow bounds visibility test"),
		OPTION("grab parent object in forge editing"),
		OPTION("never use imposter for shadow generation"),
		OPTION("Hoistable from stand"),
		OPTION("Hoistable from crouch"),
		OPTION("vaultable"),
		OPTION("collides with own projectiles"),
		OPTION("object rejects dynamic decals"),
	};

	TAG_ENUM(object_definition_secondary_flags, 2)
	{
		OPTION("does not affect projectile aiming"),
		OPTION("update state animation and keyframe rigid bodies every frame"),
	};
	...
	{
		FIELD( _field_long_flags, "ai flags", &ai_properties_flags ),
		...
		FIELD( _field_enum, "ai size", &ai_size_enum ),
		FIELD( _field_enum, "leap jump speed", &global_ai_jump_height_enum ),
		FIELD( _field_terminator )
	};

	TAG_BLOCK(object_function_block, k_maximum_object_functions)
	{
		FIELD( _field_long_flags, "flags", &object_function_flags ),
	...
	{
		FIELD( _field_long_enum, "interpolation mode", &object_function_interpolation_mode_enum ),
	...
	TAG_BLOCK(global_object_attachment_block, k_maximum_number_of_attachments_per_object)
	{
		...
		FIELD( _field_char_enum, "change color", &global_object_change_color_enum ),
		FIELD( _field_byte_flags, "flags", &object_attachment_flags ),
	...
	TAG_BLOCK(object_change_color_function, MAXIMUM_NUMBER_OF_OBJECT_CHANGE_COLOR_FUNCTIONS)
	{
		...
		FIELD( _field_long_flags, "scale flags", &global_rgb_interpolation_flags ),
		FIELD( _field_real_rgb_color, "color lower bound" ),
		FIELD( _field_real_rgb_color, "color upper bound" ),
	...
	TAG_BLOCK(multiplayer_object_block, k_maximum_multiplayer_object_blocks)
	{
		...
		FIELD( _field_char_enum, "type", &multiplayer_object_type ),
		FIELD( _field_byte_flags, "teleporter passability:used only for teleporters", &teleporter_passability_flags ),
		FIELD( _field_char_enum, "spawn timer type", &multiplayer_object_spawn_timer_types ),
		...
		FIELD( _field_real, "boundary -height" ),
		FIELD( _field_char_enum, "boundary shape", &multiplayer_object_boundary_shape ),
		...
		FIELD( _field_word_flags, "flags", &multiplayer_object_flags ),
...
TAG_STRUCT(object_struct_definition)
{
		...
		FIELD( _field_long_flags, "flags", &object_definition_flags ),
		...
		FIELD( _field_enum, "lightmap shadow mode", &lightmap_shadow_mode_enum ),
		FIELD( _field_char_enum, "sweetener size", &sweetener_size_enum ),
		FIELD( _field_char_enum, "water density", &water_density_type_enum ),
		...
		FIELD( _field_word_flags, "secondary flags", &object_definition_secondary_flags ),
...
TAG_STRUCT(object_abandonment_struct)
{
		...
		FIELD( _field_byte_flags, "flags", &object_abandonment_flags ),
