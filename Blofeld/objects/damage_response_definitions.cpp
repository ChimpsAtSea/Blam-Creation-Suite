...
namespace blofeld
{
	...
	{
		FIELD( _field_enum, "type", &damage_response_class_type_enum ),
		FIELD( _field_word_flags, "flags", &damage_response_class_flags ),
	...
	TAG_BLOCK(area_control_block, 1)
	{
		...
		FIELD( _field_word_flags, "flags", &area_control_flags ),
	...
	TAG_STRUCT(damage_response_directional_flash_struct)
	{
		...
		FIELD( _field_enum, "fade function", &global_reverse_transition_functions_enum ),
	...
	TAG_ENUM(area_control_flags, 4)
	{
		OPTION("distance_falloff"),
		OPTION("angle_falloff"),
		OPTION("object function falloff"),
		OPTION("use unit position#use the head position and facing vector of the unit instead of the player camera"),
	};

	TAG_ENUM(damage_response_class_type_enum, 3)
	{
		OPTION("shielded"),
		OPTION("unshielded"),
		OPTION("all"),
	...
	TAG_ENUM(damage_response_class_flags, 5)
	{
		OPTION("ignore on no damage"),
		OPTION("suppress directional damage flashes"),
		OPTION("suppress directional damage arrows"),
		OPTION("only when zoomed#if target is zoomed"),
		OPTION("sound effect only applied when scale is full"),
	};
