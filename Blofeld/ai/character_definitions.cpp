...
namespace blofeld
{
	...
	TAG_ENUM(generic_flags, 1)
	{
		OPTION("Flag1"),
	};

	TAG_ENUM(actor_type_enum, 22)
	{
		OPTION("none"),
		OPTION("player"),
		OPTION("marine"),
		...
		OPTION("rook"),
		OPTION("mule"),
		OPTION("mounted_weapon"),
		OPTION("octopus"),
	};

	TAG_ENUM(global_ai_grenade_type_enum, 5)
	{
		OPTION("NONE"),
		OPTION("Human grenade"),
		OPTION("Covenant Plasma"),
		OPTION("Brute claymore"),
		OPTION("Firebomb"),
	};

	TAG_ENUM(behavior_tree_root_enum, 3)
	{
		OPTION("Default"),
		OPTION("Scarab"),
		OPTION("Flying"),
	...
	TAG_ENUM(general_flags, 17)
	{
		OPTION("swarm"),
		OPTION("flying"),
		OPTION("dual wields"),
		OPTION("uses gravemind"),
		OPTION("gravemind chorus"),
		OPTION("don\'t trade weapon"),
		OPTION("don\'t stow weapon"),
		OPTION("hero character"),
		OPTION("leader independent positioning"),
		OPTION("has active camo"),
		OPTION("use head marker for looking"),
		OPTION("space character"),
		OPTION("do not drop equipment"),
		OPTION("do not allow crouch"),
		OPTION("do not allow moving crouch"),
		OPTION("critical betrayal"),
		OPTION("deathless critical betrayal"),
	};

	TAG_ENUM(combat_positioning_enum, 3)
	{
	...
	TAG_ENUM(proto_spawn_type_enum, 4)
	{
		OPTION("None"),
		OPTION("Limbo"),
		OPTION("Spawner"),
	...
	TAG_ENUM(prop_class_enum, 14)
	{
		OPTION("none - actor has no targets whatsoever"),
		OPTION("dead enemy- my only target is a dead enemy"),
		OPTION("non-attacking enemy- I am debilitated and the enemy is not attacking me"),
		OPTION("disregarded orphan- did not find my target and gave up searching"),
		OPTION("inspected orphan- checked last know position and didn\'t find target; still searching"),
		OPTION("uninspected orphan- can\'t see target, but have a good idea where they may be"),
		OPTION("certain orphan- can\'t see target, but know exactly where they are"),
		OPTION("visible enemy- I can see the target"),
		OPTION("nearby enemy- I can see the target, and it\'s close"),
		OPTION("potentially dangerous enemy- target is near and is facing me"),
		OPTION("dangerous enemy- target is facing my way and fighting"),
		OPTION("attacking enemy- target is aiming at me and shooting me"),
		OPTION("very close enemy- enemy is really close"),
		OPTION("damaging_enemy- enemy is damaging me"),
	};

	TAG_ENUM(vitality_flags, 1)
	{
		OPTION("auto resurrect"),
	};

	TAG_ENUM(perception_flags, 3)
	{
		OPTION("character can see in darkness"),
		OPTION("ignore tracking projectiles"),
		OPTION("ignore minor tracking projectiles"),
	};

	TAG_ENUM(actor_perception_mode_definition, 8)
	{
		OPTION("idle"),
		OPTION("alert"),
		OPTION("combat"),
		...
		OPTION("patrol"),
		OPTION("vehicle idle"),
		OPTION("vehicle alert"),
		OPTION("vehicle combat "),
	};

	TAG_ENUM(hopping_flags, 3)
	{
		OPTION("to cover path segements"),
		OPTION("to end of path"),
		OPTION("forward only"),
	};

	TAG_ENUM(movement_flags, 22)
	{
		OPTION("danger crouch allow movement"),
		OPTION("no side-step"),
		OPTION("prefer to combar near friends"),
		OPTION("allow boosted jump"),
		OPTION("perch"),
		OPTION("climb"),
		OPTION("prefer wall movement"),
		OPTION("has flying mode"),
		OPTION("disallow crouch"),
		OPTION("disallow all movement"),
		OPTION("always use search points"),
		OPTION("keep moving"),
		OPTION("cure isolation jump"),
		OPTION("gain elevation"),
		OPTION("reposition distant"),
		OPTION("only use aerial firing positions"),
		OPTION("use high priority pathfinding"),
		OPTION("lower weapon when no alert movement override"),
		OPTION("phase"),
		OPTION("no override when firing"),
		OPTION("no stow during idle activities"),
		OPTION("flip any vehicle"),
	};

	TAG_ENUM(obstacle_ignore_enum, 7)
	{
		OPTION("none"),
		OPTION("tiny"),
		OPTION("small"),
	...
	TAG_ENUM(movement_hint_enum, 16)
	{
		OPTION("vault step"),
		OPTION("vault crouch"),
		OPTION("unused0!"),
	...
	TAG_ENUM(global_ai_jump_height_enum, 8)
	{
		OPTION("NONE"),
		OPTION("down"),
		OPTION("step"),
	...
	TAG_ENUM(combat_status_enum_definition, 10)
	{
		OPTION("asleep"),
		OPTION("idle"),
		OPTION("alert"),
		...
		OPTION("clear enemy los"),
		OPTION("dangerous enemy"),
	};

	TAG_ENUM(movement_throttle_control_flags, 1)
	{
		OPTION("resample distance every tick"),
	};

	TAG_ENUM(character_smooth_movement_settings_options, 3)
	{
		OPTION("Use Engine Default Settings"),
		OPTION("Use Override Settings Below"),
		OPTION("Disable"),
	};

	TAG_ENUM(evaluator_enum, 69)
	{
		OPTION("INVALID&INVALID (pre)"),
		OPTION("attack_avoid_overhead&attack_avoid_overhead (pre)"),
		OPTION("attack_dangerousenemy&attack_dangerousenemy (pre)"),
		OPTION("attack_idealrange&attack_idealrange (pre)"),
		OPTION("attack_leader_distance&attack_leader_distance (pre)"),
		OPTION("attack_same_frame_of_reference&attack_same_frame_of_reference (pre)"),
		OPTION("attack_weaponrange&attack_weaponrange (pre)"),
		OPTION("combat_cue&combat_cue (pre)"),
		OPTION("combatmove_elevation&combatmove_elevation (pre)"),
		OPTION("combatmove_follow_unit&combatmove_follow_unit (pre)"),
		OPTION("combatmove_lineoffire&combatmove_lineoffire (pre)"),
		OPTION("combatmove_lineoffire_occluded&combatmove_lineoffire_occluded (pre)"),
		OPTION("cover_near_friends&cover_near_friends (pre)"),
		OPTION("current_position&current_position (pre)"),
		OPTION("current_destination&current_destination (pre)"),
		OPTION("cover_fight&cover_fight (pre)"),
		OPTION("danger_zone&danger_zone (pre)"),
		OPTION("directional_movement&directional_movement (pre)"),
		OPTION("facing&facing (pre)"),
		OPTION("fireteam_leader&fireteam_leader (pre)"),
		OPTION("flag_preferences&flag_preferences (pre)"),
		OPTION("flank&flank (pre)"),
		OPTION("flee_to_leader&flee_to_leader (pre)"),
		OPTION("friendly&friendly (pre)"),
		OPTION("friend_bunkering&friend_bunkering (pre)"),
		OPTION("goal_points_only&goal_points_only (pre)"),
		OPTION("goal_preferences&goal_preferences (pre)"),
		OPTION("heatmap&heatmap (pre)"),
		OPTION("hide_equipment&hide_equipment (pre)"),
		OPTION("idle_wander&idle_wander (pre)"),
		OPTION("inertia_preservation&inertia_preservation (pre)"),
		OPTION("leadership&leadership (pre)"),
		OPTION("movement_planning&movement_planning (pre)"),
		OPTION("move_into_danger_zone&move_into_danger_zone (pre)"),
		OPTION("move_distance&move_distance (pre)"),
		OPTION("move_localized&move_localized (pre)"),
		OPTION("multi_target&multi_target (pre)"),
		OPTION("pack_stalk&pack_stalk (pre)"),
		OPTION("panic_closetotarget&panic_closetotarget (pre)"),
		OPTION("panic_targetdistance&panic_targetdistance (pre)"),
		OPTION("pathfinding&pathfinding (pre)"),
		OPTION("perch_preferences&perch_preferences (pre)"),
		OPTION("postsearch_prefer_original&postsearch_prefer_original (pre)"),
		OPTION("previously_discarded&previously_discarded (pre)"),
		OPTION("pursuit_search_pattern&pursuit_search_pattern (pre)"),
		OPTION("ranged_fight&ranged_fight (pre)"),
		OPTION("squad_patrol_search&squad_patrol_search (pre)"),
		OPTION("stalking&stalking (pre)"),
		OPTION("stimulus&stimulus (pre)"),
		OPTION("task_direction&task_direction (pre)"),
		OPTION("threat_axis_nearby&threat_axis_nearby (pre)"),
		OPTION("threat_axis_quarter&threat_axis_quarter (pre)"),
		OPTION("threat_axis_spring&threat_axis_spring (pre)"),
		OPTION("threat_axis_spring_reject&threat_axis_spring_reject (pre)"),
		OPTION("threat_axis_third&threat_axis_third (pre)"),
		OPTION("score&score (pre)"),
		OPTION("too_far_from_leader&too_far_from_leader (pre)"),
		OPTION("vehicle_move_distance&vehicle_move_distance (pre)"),
		OPTION("vehicle_pickup&vehicle_pickup (pre)"),
		OPTION("wall_leanable&wall_leanable (pre)"),
		OPTION("3d_path_available&3d_path_available (post)"),
		OPTION("attack_visible&attack_visible (post)"),
		OPTION("attack_visible_los_optional&attack_visible_los_optional (post)"),
		OPTION("guard_cover&guard_cover (post)"),
		OPTION("hide_cover&hide_cover (post)"),
		OPTION("uncover_visible&uncover_visible (post)"),
		OPTION("vehicle_hide_cover&vehicle_hide_cover (post)"),
		OPTION("cover_fight_post&cover_fight_post (post)"),
		OPTION("obstacle&obstacle (post)"),
	};

	TAG_ENUM(evaluation_modes, 11)
	{
		OPTION("fight"),
		OPTION("panic"),
		OPTION("cover"),
		...
		OPTION("postsearch"),
		OPTION("cover_fight"),
		OPTION("cover_evasion"),
	};

	TAG_ENUM(engage_flags, 9)
	{
		OPTION("engage perch"),
		OPTION("fight constant movement"),
		OPTION("flight fight constant movement"),
		OPTION("disallow combat crouching"),
		OPTION("disallow crouch shooting"),
		OPTION("fight stable"),
		OPTION("throw should lob"),
		OPTION("allow positioning beyond ideal range"),
		OPTION("can suppress"),
	};

	TAG_ENUM(cover_flags, 2)
	{
		OPTION("Unassailable cover ends ONLY when target assailable"),
		OPTION("Use Phasing"),
	};

	TAG_ENUM(character_search_flags, 4)
	{
		OPTION("crouch on investigate"),
		OPTION("walk on pursuit"),
		OPTION("search forever"),
		OPTION("search exclusively"),
	};

	TAG_ENUM(character_retreat_flags, 2)
	{
		OPTION("Zig-zag when fleeing"),
		OPTION("Unused1"),
	};

	TAG_ENUM(charge_flags, 8)
	{
		OPTION("offhand melee allowed"),
		OPTION("berserk whenever charge"),
		OPTION("don\'t use berserk mode"),
		OPTION("don\'t stow weapon during berserk"),
		OPTION("allow dialogue while berserking"),
		OPTION("don\'t play berserk animation"),
		OPTION("don\'t shoot during charge"),
		OPTION("allow leap with ranged weapons"),
	};

	TAG_ENUM(actor_special_fire_mode_enum, 3)
	{
		OPTION("none"),
		OPTION("overcharge"),
		OPTION("secondary trigger"),
	};

	TAG_ENUM(actor_special_fire_situation_enum, 4)
	{
		OPTION("never"),
		OPTION("enemy visible"),
		OPTION("enemy out of sight"),
		OPTION("strafing"),
	};

	TAG_ENUM(weapon_flags, 6)
	{
		OPTION("bursting inhibits movement"),
		OPTION("must crouch to shoot"),
		OPTION("use extended safe-to-save range"),
		OPTION("fixed aiming vector"),
		OPTION("aim at feet"),
		OPTION("force aim from barrel position#use only for weapons with really, really long barrels (bfg), do NOT use for rotating turret weapons (warthog, falcon, etc)"),
	};

	TAG_ENUM(actor_grenade_trajectory_enum, 3)
	{
		OPTION("toss"),
		OPTION("lob"),
		OPTION("bounce"),
	...
	TAG_ENUM(grenades_flags, 2)
	{
		OPTION("do not throw while bunkering"),
		OPTION("allow while berserking"),
	};

	TAG_ENUM(vehicle_flags$2, 10)
	{
		OPTION("passengers adopt original squad"),
		OPTION("snap facing to forward (ghosts)"),
		OPTION("throttle to target (hornets)"),
		OPTION("stationary fight (tanks)"),
		OPTION("keep moving"),
		OPTION("can pathfind with avoidance only"),
		OPTION("use volume avoidance"),
		OPTION("target equality"),
		OPTION("don\'t face target"),
		OPTION("override aiming limits"),
	};

	TAG_ENUM(vehicle_boarding_flags, 1)
	{
		OPTION("boarding does not enter seat"),
	};

	TAG_ENUM(boarding_flags, 1)
	{
		OPTION("airborne boarding"),
	};

	TAG_ENUM(rook_flags, 1)
	{
		OPTION("active while socketed"),
	};

	TAG_ENUM(agent_filter_flags, 4)
	{
		OPTION("Companion"),
		OPTION("Protector"),
		OPTION("Spawner"),
	...
	TAG_ENUM(task_generation_flags, 2)
	{
		OPTION("Protect Me"),
		OPTION("Resurrect On Death"),
	};

	TAG_ENUM(character_advance_type_enum, 3)
	{
		OPTION("charge"),
		OPTION("teleport"),
		OPTION("leap"),
	...
	TAG_ENUM(advance_flags, 1)
	{
		OPTION("charge on shield down"),
	};

	TAG_ENUM(forerunner_flags, 2)
	{
		OPTION("charge on command"),
		OPTION("Teleport to combat pos"),
	};

	TAG_ENUM(character_equipment_usage_when_enum, 8)
	{
		OPTION("combat"),
		OPTION("cover"),
		OPTION("shield"),
	...
	TAG_ENUM(character_equipment_usage_how_enum, 5)
	{
		OPTION("Attach to self"),
		OPTION("Throw at enemy"),
		OPTION("Throw at feet"),
		OPTION("Use on self"),
		OPTION("Stop using on self"),
	};

	TAG_ENUM(character_equipment_def_bits, 1)
	{
		OPTION("Stop If No Energy"),
	};

	TAG_ENUM(character_flags, 1)
	{
		OPTION("Flag1"),
	};
	...
	{
		FIELD( _field_long_flags, "general flags", &general_flags ),
		FIELD( _field_enum, "type", &actor_type_enum ),
		...
		FIELD( _field_enum, "follower positioning#where should my followers try and position themselves when I am their leader\?", &combat_positioning_enum ),
		...
		FIELD( _field_enum, "default grenade type", &global_ai_grenade_type_enum ),
		FIELD( _field_enum, "behavior tree root", &behavior_tree_root_enum ),
	...
	{
		FIELD( _field_enum, "Proto Spawn Type #Can be used to automatically setup a character to be spawned by another character.", &proto_spawn_type_enum ),
	...
	{
		FIELD( _field_enum, "highest prop class^#The prop class that this block is describing", &prop_class_enum ),
	...
	{
		FIELD( _field_long_flags, "vitality flags", &vitality_flags ),
	...
	{
		FIELD( _field_enum, "perception_mode^", &actor_perception_mode_definition ),
		FIELD( _field_word_flags, "flags", &perception_flags ),
	...
	{
		FIELD( _field_long_flags, "Hopping Flags", &hopping_flags ),
	...
	{
		FIELD( _field_long_flags, "movement flags", &movement_flags ),
		...
		FIELD( _field_enum, "obstacle leap min size", &obstacle_ignore_enum ),
		FIELD( _field_enum, "obstacle leap max size", &obstacle_ignore_enum ),
		FIELD( _field_enum, "obstacle ignore size", &obstacle_ignore_enum ),
		FIELD( _field_enum, "obstacle smashable size", &obstacle_ignore_enum ),
		FIELD( _field_pad, "RQEMJY", 2 ),
		FIELD( _field_enum, "jump height", &global_ai_jump_height_enum ),
		...
		FIELD( _field_long_flags, "movement hints", &movement_hint_enum ),
	...
	{
		FIELD( _field_enum, "combat status#When combat status is bigger or equal to this combat status, use the throttle settings below.", &combat_status_enum_definition ),
		FIELD( _field_word_flags, "flags", &movement_throttle_control_flags ),
	...
	{
		FIELD( _field_long_enum, "mode^", &evaluation_modes ),
		FIELD( _field_custom ),
	...
	TAG_BLOCK(evaluator_definition_block, k_fp_evaluator_count)
	{
		...
		FIELD( _field_long_enum, "evaluator^", &evaluator_enum ),
		FIELD( _field_custom ),
	...
	{
		FIELD( _field_long_flags, "flags", &engage_flags ),
	...
	{
		FIELD( _field_long_flags, "Charge flags", &charge_flags ),
	...
	{
		FIELD( _field_long_flags, "cover flags", &cover_flags ),
	...
	{
		FIELD( _field_long_flags, "Retreat flags", &character_retreat_flags ),
	...
	{
		FIELD( _field_long_flags, "Search flags", &character_search_flags ),
	...
	{
		FIELD( _field_long_flags, "Pre-search flags", &generic_flags ),
	...
	{
		FIELD( _field_long_flags, "flags", &boarding_flags ),
	...
	TAG_BLOCK(character_vehicle_boarding_block, s_character_vehicle_boarding_properties::k_block_count)
	{
		...
		FIELD( _field_long_flags, "flags", &vehicle_boarding_flags ),
		FIELD( _field_terminator )
	};
	...
	{
		FIELD( _field_long_flags, "weapons flags", &weapon_flags ),
		FIELD( _field_tag_reference, "weapon^" ),
		...
		FIELD( _field_enum, "special-fire mode#the type of special weapon fire that we can use", &actor_special_fire_mode_enum ),
		FIELD( _field_enum, "special-fire situation#when we will decide to use our special weapon fire mode", &actor_special_fire_situation_enum ),
	...
	{
		FIELD( _field_long_flags, "grenades flags", &grenades_flags ),
		FIELD( _field_enum, "grenade type#type of grenades that we throw^", &global_grenade_type_enum ),
		FIELD( _field_enum, "trajectory type#how we throw our grenades", &actor_grenade_trajectory_enum ),
	...
	TAG_BLOCK(character_vehicle_block, c_character_vehicle_properties_internal::k_block_count)
	{
		...
		FIELD( _field_long_flags, "vehicle flags", &vehicle_flags ),
	...
	TAG_BLOCK(character_equipment_block, 8)
	{
		...
		FIELD( _field_long_flags, "flags", &character_equipment_def_bits ),
	...
	TAG_BLOCK(character_equipment_usage_block, k_character_equipment_usage_when_count)
	{
		...
		FIELD( _field_enum, "use when^#When should we use this equipment\?", &character_equipment_usage_when_enum ),
		FIELD( _field_enum, "use how#How should we use this equipment\?", &character_equipment_usage_how_enum ),
		FIELD( _field_explanation, "Skip Fraction" ),
	...
	TAG_BLOCK(character_combotron_child_block, c_character_combotron_child_properties_internal::k_block_count)
	{
		...
		FIELD( _field_long_flags, "flags", &rook_flags ),
	...
	{
		FIELD( _field_long_flags, "Agent Filter Flags #Limits dynamic tasks that can be assigned to this AI.", &agent_filter_flags ),
		FIELD( _field_long_flags, "Task Generation Flags #Controls which tasks can be generated by this AI.", &task_generation_flags ),
	...
	TAG_BLOCK(character_advance_block, c_character_advance_properties_internal::k_block_count)
	{
		...
		FIELD( _field_long_flags, "advance flags", &advance_flags ),
		...
		FIELD( _field_long_enum, "advance method", &character_advance_type_enum ),
	...
	{
		FIELD( _field_long_flags, "forerunner flags", &forerunner_flags ),
...
{
		FIELD( _field_long_flags, "Character flags", &character_flags ),
...
TAG_STRUCT(SmoothThrottleStruct)
{
		...
		FIELD( _field_enum, "settings options", &character_smooth_movement_settings_options ),
