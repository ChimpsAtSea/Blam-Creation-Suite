...
void MappingFileParser::parseImpl(std::vector<MapLine>& rRawLines)
{
	...
	write_line_verbose("MappingFileParser: Found %llu sections", uint64_t(m_sections.size()));
	for (Section& rSection : m_sections)
	{
		write_line_verbose("MappingFileParser:  %x %s %s", rSection.m_sectionIndex, rSection.m_name.c_str(), rSection.m_class.c_str());
	}

	write_line_verbose("MappingFileParser: Found %llu public symbols", uint64_t(m_publicSymbols.size()));
