...
inline MapID StringToMapID(const char* string)
{
	MapID map_id_list[] =
	{
		_map_id_mainmenu,
	...
	{
		MapID map_id = map_id_list[i];
		const char* map_id_str = MapIDToString(map_id);
...
inline const char* MapIDToString(MapID map_id)
{
	switch (map_id)
	{
...
inline int MapIDToScenarioID(MapID map_id)
{
	switch (map_id)
	{
	case _map_id_m05:
