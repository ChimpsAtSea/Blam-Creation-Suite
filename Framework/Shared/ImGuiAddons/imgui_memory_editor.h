...
struct MemoryEditor
{
	...
	bool            OptShowSizeDisplay;                     // = true   // displays the size of memory instead of range
    ...
    void DrawContents(void* mem_data_void_ptr, size_t mem_size, size_t base_display_addr = 0x0000)
    {
		...
		const char* format_size = OptUpperCaseHex ? "Size 0x%" _PRISizeT "X" : "Size 0x%" _PRISizeT "x";
        ...
        //if (OptShowOptions)
        {
            ...
            ImGui::SameLine();
            if (OptShowSizeDisplay)
            {
                ImGui::Text(format_size, mem_size);
            }
            else
            {
