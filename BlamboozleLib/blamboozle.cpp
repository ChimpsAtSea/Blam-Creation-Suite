...
int blamboozle_run()
{
	...
	if (halo1_guerilla_file.empty() && halo2_guerilla_file.empty() && halo5_forge_file.empty())
	{
	...
	{
		result += blamboozle_run(output_directory.c_str(), halo1_guerilla_file.c_str());
	}

	if (!halo2_guerilla_file.empty())
	{
		result += blamboozle_run(output_directory.c_str(), halo2_guerilla_file.c_str());
	}

	if (!halo5_forge_file.empty())
	{
		result += blamboozle_run(output_directory.c_str(), halo5_forge_file.c_str());
	}

	return result;
}

int blamboozle_run(const wchar_t* output_directory, const wchar_t* binary_filepath)
{
