...
c_h4_tag_reference::c_h4_tag_reference(const char* h4_data, const s_h4_tag_reference_definition* reference_definition, uint32_t definition_address) :
	...
	public_symbol(c_h4_blamboozle::symbols->get_public_symbol_by_base_virtual_address(definition_address))
{
	...
	{
		uint32_t value = reference_definition->group_tag;
		tag_group group = { value };
		tag_groups.push_back(group);
	}
	else if (reference_definition->group_tags_address != 0)
	{
		const uint32_t* current_group = h4_va_to_pointer<uint32_t>(h4_data, reference_definition->group_tags_address);
