...
void render_tagblock_gui_legacy(void* field_data, const c_reflection_field& reflection_field)
{
	...
	ASSERT(c_mantle_legacy_tag_editor_gui_tab::g_current_mantle_tag_tab != nullptr);
	...
	TagBlockDynamicData& rDynamicTagBlockData = c_mantle_legacy_tag_editor_gui_tab::g_current_mantle_tag_tab->get_dynamic_data<TagBlockDynamicData>(tag_block_definition);

	ImGui::Columns(1);
	ImGui::Dummy(ImVec2());
	ImGui::SameLine(c_mantle_legacy_tag_editor_gui_tab::g_current_recursion_padding + 5.0f);
	...
	if (ImGui::CollapsingHeader(reflection_field.nice_name, ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_Framed | ImGuiTreeNodeFlags_AllowItemOverlap))
	{
		...
		ImGui::Dummy(ImVec2(c_mantle_legacy_tag_editor_gui_tab::g_current_recursion_padding + 5.0f + 800.0f, 0.0f));
		ImGui::SameLine();
		ImGui::Text("Count:");
		ImGui::SameLine();
