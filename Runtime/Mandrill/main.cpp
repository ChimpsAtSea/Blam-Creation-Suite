...
static float4 background_color = { 0.16f, 0.10f, 0.16f, 1.0f };
...
)
{
	...
	BCS_RESULT rs0 = init_console();
	BCS_RESULT rs1 = init_command_line(lpCmdLine);
	BCS_RESULT rs2 = window_create(window_title, "mandrill", ULONG_MAX, ULONG_MAX, window);
	BCS_RESULT rs3 = render_context_window_create(*window, background_color, window_render_context);
	...
	if (BCS_SUCCEEDED(rs0)) rs0 = deinit_console();
