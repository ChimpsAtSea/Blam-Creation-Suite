	...
	blofeld_tag_group_struct_definition()
{
	...
	for (unsigned long structure_entry_index = 0; structure_entry_index < layout_reader.structure_definitions_chunk->entry_count; structure_entry_index++)
	{
		...
		BCS_FAIL_THROW(get_tag_struct_definition_by_persistent_identifier(structure_entry.persistent_identifier, tag_struct_definition));
	...
	c_tag_struct_chunk* structure_chunk)
{
	...
	while (true)
	{
		...
		bool is_valid = true;
		is_valid &= field_entry_name == blofeld_field_name || (*field_entry_name == 0 && blofeld_field_name == nullptr) || strcmp(field_entry_name, blofeld_field_name) == 0;
		...
		void* field_data = high_level_object.get_field_data(blofeld_field);
		ASSERT(field_size == 0 || field_data != nullptr);
...
{
	c_tag_block_chunk* tag_group_block = binary_data_chunk.find_first_chunk<c_tag_block_chunk>();
	ASSERT(tag_group_block != nullptr);
	ASSERT(tag_group_block->tag_block_chunk_header.count == 1);
	ASSERT(blofeld_tag_group_struct_definition->persistent_identifier == tag_group_block->structure_entry->persistent_identifier);

	h_object* high_level_object;
	read_tag_block_structure_to_high_level_object(*tag_group_block, 0, high_level_object);
