...
void c_fields_chunk::log_impl(c_string_data_chunk* string_data_chunk) const
{
	log_signature();
	...
	for (unsigned long index = 0; index < entry_count; index++)
	{
		...
		log_pad(); console_write_line_verbose("\t0x%08X\ttag_field_type_index:0x%08X\tmetadata:0x%08X\t'%s'", index, entry.tag_field_type_index, entry.metadata, name);
	}
}