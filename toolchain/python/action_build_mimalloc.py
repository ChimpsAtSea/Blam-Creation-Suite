...
mimalloc_build_task = MimallocBuildTask(target_cpu, mimalloc_build_shared, [cmake_task, ninja_build_task])
mimalloc_copy_tasks = [ CopyBuildTask(mimalloc_build_task.output_library, os.path.join(output, 'lib/mimalloc.lib'), [mimalloc_build_task]) ]
if mimalloc_build_shared:
    mimalloc_copy_tasks += [ CopyBuildTask(mimalloc_build_task.output_binary, os.path.join(output, 'bin/mimalloc.dll'), [mimalloc_build_task]) ]
