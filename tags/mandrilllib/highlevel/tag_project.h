...
public:
	...
	virtual BCS_RESULT get_group_by_group_tag(tag group_tag, h_tag_group*& group) const = 0;
	virtual BCS_RESULT get_tag_instances(h_tag_instance* const*& tag_instances, uint32_t& tag_instance_count) const = 0;
	virtual BCS_RESULT get_tag_groups(h_tag_group* const*& groups, uint32_t& group_count) const = 0;
