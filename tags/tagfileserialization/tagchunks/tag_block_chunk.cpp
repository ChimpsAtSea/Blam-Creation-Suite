...
BCS_RESULT c_tag_block_chunk::read_chunk(void* userdata, const void* data, bool use_read_only, bool parse_children)
{
	...
	block_entry = &reader.get_block_definition_by_index(current_block_index);
	structure_entry = &reader.get_struct_definition_by_index(block_entry->structure_entry_index);
	structure_index = block_entry->structure_entry_index;
	struct_size = reader.get_structure_size_by_index(block_entry->structure_entry_index);
	...
	DEBUG_ONLY(const char* structure_name = reader.get_string_by_string_character_index(structure_entry->string_character_index));
...
void c_tag_block_chunk::read_tag_block_structures(c_tag_file_reader& reader) const
{
...
#define write_pad()
void c_tag_block_chunk::read_structure_data(c_tag_file_reader& reader, s_tag_persist_struct_definition& structure_entry, const char* structure_data_pos, c_tag_struct_chunk* tag_struct_chunk)
{
	uint32_t metadata_child_index = 0;
	for (uint32_t field_index = structure_entry.fields_start_index;; field_index++)
	{
		s_tag_persist_field& field_entry = reader.get_field_by_index(field_index);
		s_tag_persist_field_type& field_type = reader.get_field_type_by_index(field_entry.field_type_index);

		uint32_t field_size = field_type.size;
		if (field_size == 0 || field_type.has_child_chunk)
		{
			blofeld::e_field blofeld_field_type = reader.get_blofeld_type_by_field_type_index(field_entry.field_type_index);
			...
			case blofeld::_field_block:
			{
				...
				s_tag_persist_block_definition& block_entry = reader.get_block_definition_by_index(field_entry.metadata);
				auto& structure_entry = reader.get_struct_definition_by_index(block_entry.structure_entry_index);
				//const char* block_name = reader.get_string_by_string_character_index(block_entry.string_character_index);
				//const char* struct_name = reader.get_string_by_string_character_index(structure_entry.string_character_index);
			...
			case blofeld::_field_struct:
			{
				...
				s_tag_persist_struct_definition& structure_entry = reader.get_struct_definition_by_index(structure_entry_index);
				//const char* struct_name = reader.get_string_by_string_character_index(structure_entry.string_character_index);
				uint32_t expected_children = reader.get_structure_expected_children_by_index(structure_entry_index);
				uint32_t structure_size = reader.get_structure_size_by_index(structure_entry_index);
			...
			{
				s_tag_persist_array_definition& array_entry = reader.get_array_definition_by_index(field_entry.metadata);
				uint32_t array_structure_size = reader.get_structure_size_by_index(array_entry.structure_entry_index);
