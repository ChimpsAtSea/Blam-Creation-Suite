...
{
	std::string namespace_without_semicolons = get_namespace(_namespace_suffix_mode_none);
						...
						else
						{
							...
							for (size_t group_tag_index = 0; group_tag_index < group_tags.size(); group_tag_index++)
							{
								...
								if (BCS_SUCCEEDED(tag_definition_registry_get_tag_group_by_engine_platform_build(engine_platform_build, group_tags[group_tag_index], tag_group)))
								{
									...
									stream << get_namespace(_namespace_suffix_mode_suffix_semicolon) << tag_group->group_tag_macro_symbol;
...
void c_low_level_tag_source_generator::generate_ida_header()
{
	...
	ASSERT(indent.empty());
	std::string source_code = stream.str();
	if (!output_ida_header_file_path.empty())
	{
...
void c_low_level_tag_source_generator::generate_source()
{
	...
	for (const s_tag_struct_definition* struct_definition : c_structure_relationship_node::sorted_tag_struct_definitions[engine_platform_build.engine_type])
	{
		...
		stream << ">(" << get_namespace(_namespace_suffix_mode_suffix_semicolon) << struct_definition->type_name;
		if (name_count > 1)
		{
			stream << "$" << name_count;
		}
...
void c_low_level_tag_source_generator::generate_enum_header()
{
	...
	ASSERT(indent.empty());
	std::string source_code = stream.str();
	if (!output_enum_header_file_path.empty())
	{
