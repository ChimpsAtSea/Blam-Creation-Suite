...
void halo2_update_engine_state(IGameEngine* game_engine, const std::string engine_state_str)
{
...
c_halo2_engine_state_command::c_halo2_engine_state_command() :
	g_game_engine(nullptr)
{
	c_console::register_command(k_halo2_engine_state_command_name, this);
}

c_halo2_engine_state_command::~c_halo2_engine_state_command()
{
	c_console::unregister_command(k_halo2_engine_state_command_name);
}

bool c_halo2_engine_state_command::execute_command(const std::vector<std::string> &arguments)
{
	if (arguments.empty()) return true;

	if (!arguments.front().compare(k_halo2_engine_state_command_name))
	{
		if (arguments.size() >= 2)
		{
			...
			halo2_update_engine_state(g_game_engine, engine_state);
		}
		else return false;
	}
	return true;
...
std::string c_halo2_engine_state_command::get_command_info(const std::string &topic) const
{
	if (!topic.empty())
	{
		if (!topic.compare(k_halo2_engine_state_command_name))
		{
			return
				"Update engine state\n"
