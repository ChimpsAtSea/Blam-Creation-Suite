...
template<> void byteswap_inplace(s_halo2_tools_pc64_string_list_definition& value)
{
...
template<> void byteswap_inplace(s_halo2_tools_pc64_string_list_entry_definition& value)
{
	byteswap_inplace(value.option_address);
}

c_halo2_tools_pc64_string_list_definition::c_halo2_tools_pc64_string_list_definition(c_halo2_tools_pc64_tag_definition_manager& _tag_definition_manager, ptr32 _definition_address) :
	c_blamtoozle_string_list_definition(_tag_definition_manager),
	definition_address(_definition_address),
	string_list_definition(tag_definition_manager.read_structure<s_halo2_tools_pc64_string_list_definition>(_definition_address)),
	name(),
	code_symbol_name(name)
{
	...
	const s_halo2_tools_pc64_string_list_entry_definition* option_entries = tag_definition_manager.va_to_pointer2<s_halo2_tools_pc64_string_list_entry_definition>(string_list_definition.options_address);
	for (unsigned long option_index = 0; option_index < string_list_definition.option_count; option_index++)
	{
		s_halo2_tools_pc64_string_list_entry_definition option = option_entries[option_index];
...
c_halo2_tools_pc64_string_list_definition::~c_halo2_tools_pc64_string_list_definition()
{

}

const char* c_halo2_tools_pc64_string_list_definition::get_name()
{
	return name.c_str();
}

const char* c_halo2_tools_pc64_string_list_definition::get_code_symbol_name()
{
