...
console_write_line_with_debug("%s(%i): warning CHIMP%02i: %s " format, file_path, line_number, error_type, definition_error_code_strings[error_type], __VA_ARGS__)
		...
		s_engine_platform_build engine_platform_build)
	{
		...
		if (old_struct_definition.persistent_identifier == new_struct_definition.persistent_identifier)
		{
			...
			for (;; old_field_iter++, new_field_iter++)
			{
				...
				while (execute_tag_field_versioning(*old_field_iter, engine_platform_build, ANY_TAG, tag_field_version_max, old_field_skip_count))
				{
					old_field_iter += old_field_skip_count;
					old_field_iter++; // skip the version field
				}
				uint32_t new_field_skip_count;
				while (execute_tag_field_versioning(*new_field_iter, engine_platform_build, ANY_TAG, tag_field_version_max, new_field_skip_count))
				{
	...
	BCS_RESULT definition_duplicate_check(c_tag_struct_definition_view& tag_structs_view)
	{
		...
		using t_tag_struct_definition_table = std::map<XXH64_hash_t, const s_tag_struct_definition*>;
		t_tag_struct_definition_table tag_struct_definitions_lookup_table;

		BCS_RESULT rs = BCS_S_OK;

		for (
			const s_tag_struct_definition** tag_struct_definition_iter = tag_struct_definitions;
			*tag_struct_definition_iter;
			tag_struct_definition_iter++)
		{
			const s_tag_struct_definition& tag_struct_definition = **tag_struct_definition_iter;
			XXH64_hash_t hash = XXH64(&tag_struct_definition.persistent_identifier, sizeof(s_tag_persistent_identifier), 0);
			...
			{
				const s_tag_struct_definition& previous_struct_definition = *iterator->second;
