...
std::map<ptr64, c_h3_tag_reference_definition*> tag_reference_definitions;

c_h3_tag_reference_definition::c_h3_tag_reference_definition(const char* guerilla_data, const s_h3_tag_reference_definition& tag_reference_definition) :
	group_tags(),
	...
	code_name()
{
	...
	else if (const tag* group_tag_pointer_begin = reinterpret_cast<const tag*>(h3_va_to_pointer(guerilla_data, tag_reference_definition.group_tags_address)))
	{
	...
	{
		c_h3_tag_group_definition* group_definition = c_h3_tag_group_definition::h3_get_tag_group_definition_by_group_tag(group_tag);
...
{
	std::map<ptr64, c_h3_tag_reference_definition*>::iterator tag_block_definition_iterator = tag_reference_definitions.find(virtual_address);
	...
	const s_h3_tag_reference_definition* _tag_reference_definition = reinterpret_cast<const s_h3_tag_reference_definition*>(h3_pa_to_pointer(guerilla_data, virtual_address));
	ASSERT(_tag_reference_definition != nullptr);

	c_h3_tag_reference_definition* tag_reference_definition = reinterpret_cast<c_h3_tag_reference_definition*>(new char[sizeof(c_h3_tag_reference_definition)]);

	tag_reference_definitions[virtual_address] = tag_reference_definition;

	tag_reference_definition = new(tag_reference_definition) c_h3_tag_reference_definition(guerilla_data, *_tag_reference_definition);
