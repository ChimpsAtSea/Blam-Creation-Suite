...
{
	const typed_bptr32<const char*>* options_address = h4_va_to_pointer<const typed_bptr32<const char*>>(h4_data, enum_definition->options);
	for (unsigned long option_index = 0; option_index < enum_definition->option_count; option_index++)
	{
		const typed_bptr32<const char*>& option_address = options_address[option_index];
