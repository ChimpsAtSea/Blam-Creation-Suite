...
void c_mandrill_cache_file_gui_tab::open_tag_interface_tab(c_tag_interface& tag_interface)
{
	for (c_mandrill_gui_tab* tab : child_tabs)
	{
		c_mandrill_tag_gui_tab* pTagTab = dynamic_cast<c_mandrill_tag_gui_tab*>(tab);
		if (pTagTab == nullptr) continue;
		if (&pTagTab->get_tag_interface() != &tag_interface) continue;

		next_selected_mandrill_gui_tab = tab;
		return;
	}

	c_mandrill_gui_tab* pTab = new c_mandrill_tag_gui_tab(cache_file, tag_interface, this);
	add_tab(*pTab);
	next_selected_mandrill_gui_tab = pTab;
}

c_mandrill_cache_file_gui_tab::c_mandrill_cache_file_gui_tab(c_cache_file& cache_file) :
	cache_file(cache_file),
	c_mandrill_gui_tab(cache_file.get_map_path_utf8(), cache_file.get_map_filepath_utf8()),
	render_trigger_volumes(c_command_line::has_command_line_arg("-showtriggervolumes")),
	next_selected_mandrill_gui_tab(nullptr),
...
c_mandrill_cache_file_gui_tab::~c_mandrill_cache_file_gui_tab()
{
...
void c_mandrill_cache_file_gui_tab::render_tab_menu_gui()
{
	...
	c_mandrill_gui_tab::render_tab_menu_gui();
}

void c_mandrill_cache_file_gui_tab::render_cache_file_gui()
{
...

	{
		...
		if (use_search)
		{
			c_tag_interface* const* tag_interfaces = c_mandrill_gui::get_use_full_file_length_display()
			...
			for (c_tag_interface* tag_interface : c_range_loop(tag_interfaces, cache_file.get_tag_count()))
			{
				...
				const char* tag_display_with_group_id = c_mandrill_gui::get_use_full_file_length_display()
	...
	ImGui::NextColumn();
	{
		...
		if (!child_tabs.empty()) // #NOTE: Checking this fixes strange ImGUI crash
		{
			...
			{
				c_mandrill_gui_tab* next_selected_mandrill_gui_tab_copy = next_selected_mandrill_gui_tab;
				next_selected_mandrill_gui_tab = nullptr; // take a copy as the render_gui function can set this up for the next frame
				//for (c_mandrill_gui_tab* mandrill_gui_tab : child_tabs)
				for (uint32_t i = 0; i < child_tabs.size(); i++)
				{
					c_mandrill_gui_tab* mandrill_gui_tab = child_tabs[i];
					mandrill_gui_tab->render_gui(next_selected_mandrill_gui_tab_copy == mandrill_gui_tab);
				}
		...
		for (c_mandrill_gui_tab* tab : child_tabs)
		{
			c_mandrill_tag_gui_tab* tag_tab = dynamic_cast<c_mandrill_tag_gui_tab*>(tab);
			if (tag_tab)
			{
				ImGui::SameLine();
...
void c_mandrill_cache_file_gui_tab::render_in_game_gui()
{
...
void c_mandrill_cache_file_gui_tab::render_tab_contents_gui()
{
...
void c_mandrill_cache_file_gui_tab::open_shader_tool()
{
	if (!enable_shader_tool) return;

	c_mandrill_shader_tool_gui_tab* mandrill_halo_shader_generator_gui_tab = nullptr;
	for (c_mandrill_gui_tab* mandrill_gui_tab : child_tabs)
	{
		mandrill_halo_shader_generator_gui_tab = dynamic_cast<c_mandrill_shader_tool_gui_tab*>(mandrill_gui_tab);

		if (mandrill_halo_shader_generator_gui_tab)
		{
			break;
		}
	}

	if (mandrill_halo_shader_generator_gui_tab == nullptr)
	{
		mandrill_halo_shader_generator_gui_tab = new c_mandrill_shader_tool_gui_tab("Shader Tool", "Shader Tool", *this, cache_file);

		add_tab(*mandrill_halo_shader_generator_gui_tab);
	}

	next_selected_mandrill_gui_tab = mandrill_halo_shader_generator_gui_tab;
}

void c_mandrill_cache_file_gui_tab::open_halo_script_editor()
{
	if (halo_script_editor != nullptr)
	{
		return;
	...
	halo_script_editor = new c_mandrill_halo_script_editor(*this, cache_file);
	halo_script_editor->add_tab_closed_callback([this](c_mandrill_gui_tab& hsc_tab)
		{
			halo_script_editor = nullptr;
		});
...
void c_mandrill_cache_file_gui_tab::close_halo_script_editor()
{
	if (halo_script_editor == nullptr)
	{
		return;
...
void c_mandrill_cache_file_gui_tab::render_file_dialogue_gui()
{
	...
	c_mandrill_gui_tab::render_file_dialogue_gui();
}
