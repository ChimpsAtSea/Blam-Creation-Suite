...
c_mandrill_tag_gui_tab::c_mandrill_tag_gui_tab(c_cache_file& cache_file, c_tag_interface& tag_interface, c_mandrill_gui_tab* parent_tab) : 
	c_mandrill_gui_tab(tag_interface.get_name_with_group_id_cstr(), tag_interface.get_path_with_group_name_cstr()),
	...
	parent_tab(parent_tab)
{
	...
	{
		c_mandrill_blofeld_tag_editor_gui_tab* blofeld_tag_editor_gui_tab = new c_mandrill_blofeld_tag_editor_gui_tab(cache_file, this, tag_interface);
		ASSERT(blofeld_tag_editor_gui_tab != nullptr);
		add_tab(*blofeld_tag_editor_gui_tab);
	}
}

c_mandrill_tag_gui_tab::~c_mandrill_tag_gui_tab()
{
...
//void c_mandrill_tag_gui_tab::copy_data_recursively(const s_reflection_structure_type_legacy& reflection_type, char* source, char* destination)
//{
...
//					char* pTagBlockDataDest = c_mandrill_gui::get_tag_selection_address(tag_block->address);
//
...
void c_mandrill_tag_gui_tab::send_to_game()
{
	char* pDest = static_cast<char*>(c_mandrill_gui::get_tag_pointer(get_tag_interface().get_index()));
	if (pDest)
	{
...
void c_mandrill_tag_gui_tab::render_tab_contents_gui()
{
	if (child_tabs.empty())
	{
	...
	if (ImGui::BeginTabBar("##TagEditorTabs", ImGuiTabBarFlags_None)) // each tag
	{
		...
		{
			mandrill_gui_tab->render_gui(false);
		}
...
void c_mandrill_tag_gui_tab::RenderButtons()
{
