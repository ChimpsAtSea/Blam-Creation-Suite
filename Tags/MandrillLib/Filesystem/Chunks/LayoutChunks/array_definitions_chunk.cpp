...
c_array_definitions_chunk::c_array_definitions_chunk(c_chunk& parent) :
...
BCS_RESULT c_array_definitions_chunk::read_chunk(void* userdata, const void* data, bool use_read_only, bool parse_children)
{
...
void c_array_definitions_chunk::log_impl(c_tag_file_string_debugger* string_debugger) const
{
	log_signature();
	...
	for (unsigned long index = 0; index < entry_count; index++)
	{
		...
		{
			log_pad(); console_write_line_verbose("\t0x%08X\tcount:0x%08X structure_entry_index:0x%08X\tstring_character_index:0x%08X", index, entry.count, entry.structure_entry_index, entry.string_character_index.offset);
		}
	}
}

BCS_RESULT c_array_definitions_chunk::set_data(const void* data, unsigned long data_size)
{
	BCS_RESULT rs = BCS_S_OK;
...
void c_array_definitions_chunk::read_entries()
{
	if (entries)
	{
		delete[] entries;
	...
	if (entry_count > 0)
	{
		const s_tag_persist_array_definition* src_entries = reinterpret_cast<const s_tag_persist_array_definition*>(get_chunk_data_start());
