...
template<> void byteswap_inplace(s_tag_persist_interop_definition& value)
{
...
void c_interop_definitions_chunk::log_impl(c_tag_file_string_debugger* string_debugger) const
{
	log_signature();
	...
	for (unsigned long index = 0; index < entry_count; index++)
	{
		...
		{
			log_pad(); console_write_line_verbose(
				"\t0x%08X\t[0x%08X, 0x%08X, 0x%08X, 0x%08X]\tstructure_entry_index:0x%08X\tstring_character_index:0x%08X",
				index,
				entry.persistent_identifier.identifier_part_0,
				entry.persistent_identifier.identifier_part_1,
				entry.persistent_identifier.identifier_part_2,
...
BCS_RESULT c_interop_definitions_chunk::set_data(const void* data, unsigned long data_size)
{
	BCS_RESULT rs = BCS_S_OK;
...
void c_interop_definitions_chunk::read_entries()
{
	if (entries)
	{
		delete[] entries;
