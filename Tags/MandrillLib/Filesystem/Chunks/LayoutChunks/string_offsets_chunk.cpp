...
template<> void byteswap_inplace(s_tag_persist_string_character_index& value)
{
	byteswap_inplace(value.offset);
}

c_string_offsets_chunk::c_string_offsets_chunk(c_chunk& parent) :
...
BCS_RESULT c_string_offsets_chunk::read_chunk(void* userdata, const void* data, bool use_read_only, bool parse_children)
{
...
void c_string_offsets_chunk::log_impl(c_tag_file_string_debugger* string_debugger) const
{
	log_signature();
...
BCS_RESULT c_string_offsets_chunk::set_data(const void* data, unsigned long data_size)
{
	BCS_RESULT rs = BCS_S_OK;
...
void c_string_offsets_chunk::read_entries()
{
	if (offsets)
	{
		delete[] offsets;
