...
public:
	...
	unsigned long enqueue_block_definition(const blofeld::s_tag_block_definition& tag_block_definition);
	unsigned long enqueue_struct_definition(const blofeld::s_tag_struct_definition& tag_struct_definition);
	void enqueue_field(const blofeld::s_tag_field& field, s_tag_persist_field& tag_persist_field);
	unsigned long enqueue_field_type(blofeld::e_field field_type);
	unsigned long enqueue_array_definition(const blofeld::s_tag_array_definition& tag_array_definition);
	unsigned long enqueue_string_list_definition(const blofeld::s_string_list_definition& string_list_definition);
	...
	void serialize_tag_struct(const h_object& object, char* const structure_data, c_tag_struct_chunk* tag_struct_chunk);
	void serialize_tag_data(const h_data& data, c_tag_struct_chunk& parent_chunk);
	void serialize_tag_resource(const h_resource& data, c_tag_struct_chunk& parent_chunk);
