...
uint32_t c_blofeld_tag_definition_debug_tab::render_tag_struct_definition(int level, char* data, const blofeld::s_tag_struct_definition& struct_definition)
{
	...
	//		for (uint32_t option_index = 0; option_index < string_list_definition->count(engine_platform_build); option_index++)
	//		{
	//			ImGui::Dummy({ static_cast<float>(level + 1) * indent, 0.0f  });
	//			ImGui::SameLine();
	//			ImGui::Text("string> %s", string_list_definition->strings(engine_platform_build)[option_index]);
	//		}
	//		ImGui::PopStyleColor(1);
