...
void render_tagblock_gui_legacy(void* field_data, const c_reflection_field_legacy& reflection_field)
{
	...
	TagBlockDynamicData& rDynamicTagBlockData = c_mandrill_legacy_tag_editor_gui_tab::g_current_mandrill_tag_tab->get_dynamic_data<TagBlockDynamicData>(tag_block_definition);

	ImGui::Columns(1);
	ImGui::Dummy(ImVec2());
	ImGui::SameLine(c_mandrill_legacy_tag_editor_gui_tab::g_current_recursion_padding + 5.0f);
	...
	if (ImGui::CollapsingHeader(reflection_field.nice_name, ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_Framed | ImGuiTreeNodeFlags_AllowItemOverlap))
	{
		...
		ImGui::Dummy(ImVec2(c_mandrill_legacy_tag_editor_gui_tab::g_current_recursion_padding + 5.0f + 800.0f, 0.0f));
		ImGui::SameLine();
		ImGui::Text("Count:");
		ImGui::SameLine();
		...
		if (tag_block_definition->count && tag_block_definition->address)
		{
			...
			char* tag_block_data_start = c_mandrill_legacy_tag_editor_gui_tab::g_current_mandrill_tag_tab->cache_file.get_tag_block_data<char>(*tag_block_definition);
			...
			else
			{
				...
				if (tag_block_reflection_type)
				{
					...
					tag_block_reflection_type->render_type_gui_legacy(tag_block_data);
