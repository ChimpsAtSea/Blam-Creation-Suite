...
void* c_virtual_tag_interface::get_field_pointer(const blofeld::s_tag_field& field)
{
	...
	for (const s_tag_field* current_field = struct_definition.fields; current_field->field_type != _field_terminator; current_field++)
	{
		...
		case _field_skip:
			...
			break;
		case _field_struct:
			current_position += calculate_struct_size(engine_type, platform_type, build, *current_field->struct_definition);
			break;
		case _field_array:
		{
			...
			uint32_t struct_size = calculate_struct_size(engine_type, platform_type, build, struct_definition);
