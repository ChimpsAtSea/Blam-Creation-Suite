...
namespace blofeld
{
	...
	V5_TAG_STRUCT(crate_struct_definition)
	{
		...
		{ _field_legacy, _field_word_flags, "flags", &crate_flags },
		{ _field_legacy, _field_pad, "KOQVK", 2 },
		{ _field_legacy, _field_block, "campaign metagame bucket", &campaign_metagame_bucket_block_block },
		{ _field_legacy, _field_long_integer, "self destruction timer:seconds" },

		{ _field_legacy, _field_version_greater, _engine_type_haloreach, 5 },
		{ _field_legacy, _field_tag_reference, "particleize#optional particleization effect definition, if you want this to particleize when it takes damage", &global_particleize_parameters_reference },
		{ _field_legacy, _field_string_id, "grab animation set#the animation set to use when this crate is grabbed" },
		{ _field_legacy, _field_string_id, "grab pickup string#the string to display when the player can grab this object, from ui/hud/hud_messages" },
		{ _field_legacy, _field_tag_reference, "projectile bounce effect#effect to play when a projectile bounces because of the \"all projectiles bounce off\" flag", &global_effect_reference },
