	...
	s_engine_platform_build engine_platform_build)
{
	...
	if (old_string_list_count == new_string_list_count)
	{
		for (unsigned long string_index = 0; string_index < old_string_list_count; string_index++)
		{
			...
			{
				console_write_line("%s(%i): warning DM05: [0] string list mismatch %s -> %s", old_string_list_definition.filename, old_string_list_definition.line, old_string, new_string);
				console_write_line("%s(%i): warning DM05: [1] string list mismatch %s -> %s", new_string_list_definition.filename, new_string_list_definition.line, old_string, new_string);
	...
	{
		console_write_line("%s(%i): warning DM05: [0] string list count mismatch %u -> %u", old_string_list_definition.filename, old_string_list_definition.line, old_string_list_count, new_string_list_count);
		console_write_line("%s(%i): warning DM05: [1] string list count mismatch %u -> %u", new_string_list_definition.filename, new_string_list_definition.line, old_string_list_count, new_string_list_count);
	}
	...
	s_engine_platform_build engine_platform_build)
{
	...
	if (old_struct_definition.persistent_identifier == new_struct_definition.persistent_identifier)
	{
		...
		{
			if (old_field_iter->name && strcmp(old_field_iter->name, "mesh resource") == 0)
			{
				debug_point;
			}
			...
			if (old_field.field_type != new_field.field_type)
			{
				...
				console_write_line("%s(%i): warning DM04: [0] structure field missmatch [%s] [%s %s] -> [%s %s]", old_field.filename, old_field.line, new_struct_definition.name, old_field_type_name, old_field.name, new_field_type_name, new_field.name);
				console_write_line("%s(%i): warning DM04: [1] structure field missmatch [%s] [%s %s] -> [%s %s]", new_field.filename, new_field.line, old_struct_definition.name, old_field_type_name, old_field.name, new_field_type_name, new_field.name);
				result = true;
				break;
			}
			...
			{
				console_write_line("%s(%i): warning DM06: [0] structure field name missmatch [%s] %s -> %s", old_field.filename, old_field.line, new_struct_definition.name, old_field.name, new_field.name);
				console_write_line("%s(%i): warning DM06: [1] structure field name missmatch [%s] %s -> %s", new_field.filename, new_field.line, old_struct_definition.name, old_field.name, new_field.name);
				result = true;
				break;
			}
			...
			case _field_pageable:
				if (old_field.struct_definition) // #TODO: export flight 1 pageables again
				{
					...
					{
						console_write_line("%s(%i): warning DM03: [0] structure missmatch null", old_field.filename, old_field.line);
						console_write_line("%s(%i): warning DM03: [1] structure missmatch null", new_field.filename, new_field.line);
						result = true;
					}
					else
					{
			...
			case _field_struct:
				...
				{
					console_write_line("%s(%i): warning DM03: [0] structure missmatch %s -> %s", old_field.filename, old_field.line, old_field.struct_definition->name, new_field.struct_definition->name);
					console_write_line("%s(%i): warning DM03: [1] structure missmatch %s -> %s", new_field.filename, new_field.line, old_field.struct_definition->name, new_field.struct_definition->name);
					result = true;
				}
				else
				{
	...
	{
		console_write_line("%s(%i): warning DM03: structure missmatch %s -> %s", new_struct_definition.filename, new_struct_definition.line, old_struct_definition.name, new_struct_definition.name);
	}
	...
	s_engine_platform_build engine_platform_build)
{
	...
	{
		console_write_line("%s(%i): warning DM02: [0] new block count incorrect for old version %u -> %u", old_block_definition.filename, old_block_definition.line, old_block_count, new_block_count);
		console_write_line("%s(%i): warning DM02: [1] new block count incorrect for old version %u -> %u", new_block_definition.filename, new_block_definition.line, old_block_count, new_block_count);
	}
	...
	s_engine_platform_build engine_platform_build)
{
	...
	for (const s_tag_group** old_tag_groups_iter = old_tag_groups; *old_tag_groups_iter; old_tag_groups_iter++)
	{
		...
		console_write_line("%s(%i): warning DM01: tag group missing", old_tag_group.filename, old_tag_group.line, old_tag_group.name);
