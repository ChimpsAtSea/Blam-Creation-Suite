...
public:
	...
	BCS_SHARED c_tag_block_chunk(c_chunk& parent);
	BCS_SHARED ~c_tag_block_chunk();
	...
	BCS_SHARED const char* get_sturcutre_data_by_index(uint32_t index) const;
	BCS_SHARED c_tag_struct_chunk* get_sturcutre_chunk_by_index(uint32_t index) const;

	BCS_SHARED void read_tag_block_structures(c_tag_file_reader& reader) const;
	BCS_SHARED void read_structure_data(c_tag_file_reader& reader, s_tag_persist_struct_definition& structure_entry, const char* structure_data_pos, c_tag_struct_chunk* tag_struct_chunk);
