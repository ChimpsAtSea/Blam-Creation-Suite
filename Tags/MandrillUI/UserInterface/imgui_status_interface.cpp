...
const char* c_imgui_status_interface::get_status_bar_text()
{
	...
	if (remaining_time < 0)
	{
		if (status_bar_state == 0 && atomic_cmpxchgu32(&status_bar_state, 1, 0) == 0)
		{
			strcpy(status, "Ready");
			end_time = INFINITY;
			priority = _status_interface_priority_low;
			atomic_xchgu32(&status_bar_state, 2);
		}
	}
	if (status_bar_state == 2 && atomic_cmpxchgu32(&status_bar_state, 1, 2) == 2)
	{
		strcpy(status_ui_copy, status);
		atomic_xchgu32(&status_bar_state, 0);
	}
