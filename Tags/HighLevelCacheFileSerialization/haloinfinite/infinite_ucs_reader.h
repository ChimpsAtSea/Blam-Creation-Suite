...
{
	int32_t magic;								// 'hscu' (UCSH = "Universal Cache Storage Header" ? )
	int32_t version;							// 27 (0x1B)
	uint64_t unknown8;
	uint64_t asset_checksum;		// Murmur3_x64_128 hash of(what appears to be) the original file that this file was built from.This is not always the same thing as the file stored in the module.
	//int32_t unknown18;
	uint32_t tag_dependency_count;		// Number of tag dependencies
	uint32_t nugget_count;				// Number of data blocks
	uint32_t tag_block_count;			// Number of tag structs
	uint32_t data_reference_count;		// Number of data references
	uint32_t tag_reference_count;		// Number of tag references
	//int32_t string_id_count;					// Number of string_ids
	uint32_t string_table_size;		// String table size in bytes
	uint32_t zoneset_data_size;		// Zoneset data size in bytes
	int32_t unknown19;
	uint32_t header_size;				// Header size in bytes
	uint32_t data_size;				// Tag data size in bytes
	uint32_t resource_data_size;		// Resource data size in bytes
	int32_t unknown18;
...
struct s_infinite_ucs_tag_dependency_list
{
	...
	uint32_t name_offset;		// Offset of the tag filename in the String Table.
	uint64_t asset_id;	// Asset ID of the tag.
	int32_t global_id;					// Global ID of the tag.
	int32_t unknown14;					// (Haven't seen this be anything other than -1 yet)
};
...
{
	uint32_t size; // The size of the data block in bytes.
	...
	uint64_t offset; // The offset of the start of the data block, relative to the start of its section.
};
...
struct s_infinite_ucs_tag_block_data
{
	...
	int32_t nugget_index;											// For Main Struct and Tag Block structs, the index of the block containing the struct.For Resource structs, this (probably)is the index of the resource.This can be - 1 if the tag field doesn't point to anything (null Tag Blocks or Custom structs).
	int32_t owner_block_index;										// The offset of the tag field inside the data block. (Unlike in Halo Online, this points to the tag field, not the pointer inside the tag field.Some tag fields for structs don't even have a pointer.)
	int32_t owner_block_offset;									// The index of the data block containing the tag field which refers to this struct.Can be - 1 for the Main Struct.
};
...
{
	int32_t parent_tag_block_index;		//The index of the tag struct containing the tag field. (See the Tag Struct List file section.)
	int32_t unknown4;						
	int32_t nugget_index;					//The index of the data block containing the referenced data.Can be - 1 for null references.
	int32_t owner_block_index;				//The index of the data block containing the tag field.
	uint32_t owner_block_offset;	//The offset of the tag field inside the data block.
};
...
{
	int32_t fieldBlock;				// The index of the data block containing the tag field.
	uint32_t fieldOffset;		// The offset of the tag field inside the data block. (Unlike in Halo Online, this points to the tag field, not the handle inside the tag field.)
	uint32_t nameOffset;		// The offset of the tag filename within the String Table.
	int32_t dependencyIndex;			// The index of the tag dependency in the Tag Dependency List.Can be - 1 for null tag references.
};
...
{
	uint32_t unknown0;			//  (Flags ? )
	uint32_t stringOffset;		//  The offset of the string value within the String Table.
};
...
{
	uint64_t elements;	// Runtime: pointer to the block containing the first struct.
	uint64_t type_info;	// Runtime: pointer to type information.
...
{
	uint64_t data;		// Runtime : pointer to the referenced data.
	uint64_t type_info;	// Runtime : pointer to type information.
	int32_t unknown10;					// (Unused ? )
	uint32_t size;				// Size of the referenced data in bytes.
};
...
struct s_infinite_ucs_tag_reference_field
{
	uint64_t type_info;	// Runtime : pointer to type information.
	int32_t global_id;					// Global ID of the tag.
	int64_t global_id64;			// Global ID of the tag.
	int32_t group_tag;					// Group tag(e.g.proj).
	int32_t local_handle;				// Runtime : the resolved local tag handle.
};
...
{
	uint64_t type_info;	// Runtime: pointer to type information.
	uint32_t handle;			// Runtime: local resource handle.
	int32_t unknownC;					// (Padding ? )
};
...
public:
	...
	int32_t root_tag_block_entry_index;
