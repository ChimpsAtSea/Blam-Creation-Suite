...
c_tag_source_generator::c_tag_source_generator(e_engine_type engine_type, e_platform_type platform_type, e_build build) :
...
const char* c_tag_source_generator::field_type_to_low_level_source_type(e_platform_type platform_type, e_field field_type)
{
	switch (field_type)
	{
...
const char* c_tag_source_generator::field_type_to_high_level_source_type(e_platform_type platform_type, blofeld::e_field field_type)
{
	switch (field_type)
	{
...
void c_tag_source_generator::generate_source()
{
	...
	{
		stream << "\t\t" << "struct s_" << tag_struct_definition->name << std::endl;
		stream << "\t\t" << "{" << std::endl;
		...
		for (const s_tag_field* current_field = tag_struct_definition->fields; current_field->field_type != _field_terminator; current_field++)
		{
			...
			case _field_block:
			{
				...
				stream << "\t\t\t" << "c_typed_tag_block<s_" << field_source_type << "> " << field_formatter.code_name.c_str() << ";";
				break;
			}
