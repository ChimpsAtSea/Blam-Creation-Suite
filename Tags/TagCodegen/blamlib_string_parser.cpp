...
#define FLAGS_SEARCH_PATTERN " %[^][:|#] "
		...
		int search_for_name_and_alt_name = sscanf(read_position, NAME_SEARCH_PATTERN OLD_NAME_SEARCH_PATTERN OLD_NAME_SEARCH_PATTERN FLAGS_SEARCH_PATTERN, name.data, old_name.data, old_name2.data, flags.data);
		...
		if (search_for_name_and_alt_name >= 3) // found old_name
		{
			read_position = strstr(read_position, old_name2) + strlen(old_name2);
			read_position += 1; // ]
		}
		else *old_name2 = 0;
		if (search_for_name_and_alt_name >= 4) // found flags
		{
...
#define LIMITS_SEARCH_PATTERN " %[^|#] "
		int search_for_description_and_units = sscanf(read_position, ":" UNIT_SEARCH_PATTERN LIMITS_SEARCH_PATTERN, units.data, limits.data);

		read_position += 1; // colon
		if (search_for_description_and_units == 0) // couldn't find units, check for limits
		{
			int search_for_limits = sscanf(read_position, LIMITS_SEARCH_PATTERN, limits.data);
...
#define META_SEARCH_PATTERN "%[^][#]"
		int search_for_meta = sscanf(read_position, "|" META_SEARCH_PATTERN, meta.data);
...
#define DESCRIPTION_SEARCH_PATTERN " %[^\xFF] "
		int search_for_description = sscanf(read_position, "#" DESCRIPTION_SEARCH_PATTERN, description.data);
