...
public:
	...
	void load_map_async(const std::wstring& map_filepath);
	...
	const s_section_cache& get_section(gen3::e_cache_file_section_index cache_file_section) const;
	...
	{ 
		return static_cast<gen3::s_cache_file_tag_instance*>(get_internal_tag_instance_impl(tag_index));
	}
	inline gen3::s_cache_file_tag_group* get_internal_tag_group(uint32_t group_index) const
	{
		if (group_index == blofeld::INVALID_TAG)
		{
			return nullptr;
		}
		return static_cast<gen3::s_cache_file_tag_group*>(get_internal_tag_group_impl(group_index));
	}
...
protected:
	...
	haloreach::s_haloreach_cache_file_header* cache_file_header;
	haloreach::s_cache_file_tags_header* cache_file_tags_header;
