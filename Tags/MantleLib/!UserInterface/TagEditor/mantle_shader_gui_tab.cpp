...
c_mantle_shader_gui_tab::c_mantle_shader_gui_tab(c_cache_file& cache_file, c_mantle_gui_tab* parent_tab, v_tag_interface_legacy<s_shader_definition_legacy>& shader_tag_interface) :
...
void c_mantle_shader_gui_tab::render_tab_contents_gui()
{
	...
	s_shader_definition_legacy::s_shader_properties_definition_legacy& shader_properties = shader_tag_interface.shader_properties_block[0];
	
	uint32_t option_index = 0;
	uint32_t shader_map_index = 0;
	for (s_shader_definition_legacy::s_unknown_definition_legacy& unknown : shader_tag_interface.__unknown1)
	{
			...
			if (rmop && ImGui::CollapsingHeader(buffer, ImGuiTreeNodeFlags_DefaultOpen | ImGuiTreeNodeFlags_Framed | ImGuiTreeNodeFlags_AllowItemOverlap))
			{
				...
				for (s_render_method_option_definition_legacy::s_unknown_0_definition_legacy& option : rmop->unknown_0_block)
				{
					...
					if (rmop_default_bitm != nullptr)
					{
						...
						c_virtual_tag_block_legacy<s_shader_definition_legacy::s_shader_properties_definition_legacy::s_shader_maps_definition_legacy> shader_map_block(cache_file, shader_tag_interface, shader_properties.shader_maps_block);


						s_shader_definition_legacy::s_shader_properties_definition_legacy::s_shader_maps_definition_legacy& shader_map = shader_map_block[(size_t)shader_map_index];
