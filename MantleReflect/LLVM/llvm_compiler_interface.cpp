...
// llvm/clang things
static llvm::cl::OptionCategory s_mantle_tool_category("mantle options");
static LangOptions const k_clang_language_options;
static PrintingPolicy const k_clang_printing_policy = k_clang_language_options;
	...
	static_wide_string_type()
{
	...
	{
		c_reflection_type_container& rType = *reflection_type_container;
		init_type_size_and_offsets(rType);
		if (rType.is_structure && rType.data_size == 0)
		{
			assert(rType.data_size > 0);
		}
		create_nice_type_names(rType);
	}
}
...
e_primitive_type c_llvm_compiler_interface::qualified_type_to_primitive_type(const clang::QualType& _qualifiedType)
{
...
c_reflection_type_container* c_llvm_compiler_interface::create_reflected_enum_type(ASTContext* Context, const clang::QualType* recordQualifiedType, const clang::RecordDecl& rRecordDeclaration)
{
	...
	{
		qualifiedDeclarationName = QualType::getAsString(recordQualifiedType->split(), k_clang_printing_policy);
	}
...
c_reflection_type_container* c_llvm_compiler_interface::create_reflected_type(
	...
	bool is_primitive)
{
	...
	for (FieldDecl* field : fields)
	{
		...
		if (reflectionQualifiedType->isEnumeralType())
		{
			...
			c_reflection_type_container* pType = create_reflected_enum_type(ast_context, &fieldQualifiedType, *pCXXRecord);
			assert(pType != nullptr);
			rFieldData.field_type = pType;
...
void c_llvm_compiler_interface::init_primitive_types()
{
...
c_reflection_type_container* c_llvm_compiler_interface::get_primitive_reflection_type_container(e_primitive_type primitive_type)
{
	switch (primitive_type)
	{
...
uint32_t c_llvm_compiler_interface::init_type_size_and_offsets(c_reflection_type_container& reflection_type_container)
{
...
void c_llvm_compiler_interface::create_nice_type_names(c_reflection_type_container& rType)
{
...
const char* c_llvm_compiler_interface::format_nice_name_and_is_hidden_property(e_reflection_type_category reflection_type_category, char* pString, bool* is_hidden_by_default)
{
